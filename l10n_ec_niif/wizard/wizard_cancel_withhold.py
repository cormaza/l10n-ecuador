    ...
    def action_cancel_withholding(self):
        ...
        for line in self.line_ids:
            ...
            agency_recs = agency_model.search(
                [
                    ("number", "=", document_number[0]),
                    ("company_id", "=", self.company_id.id),
                ]
            )
            if not agency_recs:
                raise UserError(
                    _("Cannot find a Agency with SRI number: %(agency)s, please check")
                    % {"agency": document_number[0]}
                )
            agency_id = agency_recs[0].id
            printer_recs = printer_model.search(
                [("number", "=", document_number[1]), ("agency_id", "=", agency_id)]
            )
            if not printer_recs:
                raise UserError(
                    _(
                        "Cannot find a Point emission with number: %(number)s for agency %(agency)s, please check"
                    )
                    % {"number": document_number[1], "agency": document_number[0]}
                )
            ...
            if not retention_recs:
                ...
                if point_emission.type_emission != "electronic" and not doc_find:
                    msj.append(
                        _(
                            "Cannot find a authorization for withholding for number: %(document_number)s."
                        )
                ...
                for ret in retention_recs:
                    if ret.invoice_id:
                        if ret.invoice_id.state not in (
                            ...
                            msj.append(
                                "Already exists  a withholding with number: %(document_number)s associated to invoice: %(invoice)s for supplier: %(supplier)s."
                                % {
                                    "document_number": ret.l10n_ec_get_document_number(),
                                    "invoice": ret.invoice_id.l10n_ec_get_document_number(),
                                    "supplier": ret.invoice_id.partner_id.display_name,
                                }
                            )
                    else:
                        msj.append(
                            _("Already exists  a withholding with number %s.")
...
class WizardCancelWithholdLine(models.TransientModel):
    ...
    document_number = fields.Char(size=17, required=True)
        ...
        for line in self:
            if line.document_number and not re.match(cadena, line.document_number):
                raise ValidationError(
                    _(
                        "The document number: %(document_number)s is not valid, must be as 00X-00X-000XXXXXX, Where X is a number"
                    )
                    % {"document_number": line.document_number}
                )
