...
class WizardCancelElectronicDocuments(models.TransientModel):
    ...
    move_id = fields.Many2one(
        comodel_name="account.move", string="Move to cancel", required=False
    )
    withholding_id = fields.Many2one(
        comodel_name="l10n_ec.withhold", string="Withholding to cancel", required=False
    )
        ...
        for wizard in self:
            ...
            if len(wizard.authorization_to_cancel) not in (10, 37, 49):
                raise ValidationError(
                    _(
                        "The authorization number is incorrect, This must be 10, 37 or 49 digits."
                    )
                )
            if not re.match(cadena, wizard.authorization_to_cancel):
                raise ValidationError(
                    _("The electronic authorization must have only numbers")
                )
    ...
    def _cancel_invoice(self):
        ...
        if self.move_id.l10n_ec_credit_note_ids.filtered(
            lambda x: x.state not in ("cancel",)
        ):
            raise UserError(
                _(
                    "You cannot cancel the invoice, is used in credit notes, please cancel credit notes first!"
                )
            )
        ...
        if self.move_id.l10n_ec_xml_data_id:
            if self.move_id.l10n_ec_xml_data_id.state == "waiting":
                raise UserError(
                    _(
                        "Electronic document is waiting authorization, please try in few minutes"
                    )
                )
    ...
    def action_cancel(self):
        ...
        if self.withholding_id:
            if (
                ...
                raise UserError(
                    _(
                        "You cannot cancel a Withholding Approved was invoice related is Paid"
                    )
                )
