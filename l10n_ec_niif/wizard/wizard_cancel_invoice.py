...
class WizardCancelInvoice(models.TransientModel):
    ...
    line_ids = fields.One2many(
        "wizard.cancel.invoice.line", "wizard_id", "Document Cancel", required=True
    )
    date = fields.Date(
        "Date cancel",
        required=True,
    ...
    account_line_id = fields.Many2one(
        comodel_name="account.account", string="Account line", required=True
    )
    ...
    def action_cancel_invoice(self):
        ...
        for line in self.line_ids:
            ...
            invoice_recs = account_move_model.search(
                domain + [("l10n_ec_document_number", "=", line.number)]
            )
            number = line.number.split("-")
            company_id = self.company_id
            shop_recs = shop_model.search(
                [("number", "=", number[0]), ("company_id", "=", company_id.id)],
                limit=1,
            )
            if not shop_recs:
                raise UserError(
                    _("No existe una agencia con el numero %s, por favor verifique")
                    % (number[0])
                )
            printer_recs = printer_model.search(
                [("number", "=", number[1]), ("agency_id", "=", shop_recs.id)], limit=1
            )
            if not printer_recs:
                raise UserError(
                    _(
                        "No existe un punto de emisión con el numero %(number)s en la agencia %(agency)s, por favor verifique"
                    )
                    % {"number": number[1], "agency": number[0]}
                )
            if not invoice_recs:
                auth_line_id = printer_recs.get_authorization_for_number(
                    internal_type,
                    line.number,
                    emission_date=line.date or self.date,
                    company=company_id,
                )
                ctx = {
                ...
                new_invoice.message_post(
                    body=_("Document create as cancel from wizard")
                )
        ...
        if msj:
            raise UserError("\n".join(msj))
        model_data_recs = obj_model.search(
            [("model", "=", "ir.ui.view"), ("name", "=", "view_invoice_tree")]
        )
        ...
        for rec in self:
            if not re.match(cadena, rec.number):
                raise ValidationError(
                    _(
                        "El número de factura es incorrecto, este debe tener la forma 001-00X-000XXXXXX, X es número"
                    )
                )
