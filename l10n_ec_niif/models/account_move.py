...
class AccountMove(models.Model):
    ...
    def _compute_l10n_ec_is_environment_production(self):
        ...
        for invoice in self:
            if invoice.is_invoice():
                ...
                invoice.l10n_ec_is_environment_production = (
                    xml_model.l10n_ec_is_environment_production(
    ...
    def _compute_l10n_latam_document_type(self):
        res = super(AccountMove, self)._compute_l10n_latam_document_type()
        for move in self.filtered(
            lambda x: x.state == "draft" and x.company_id.country_id.code == "EC"
        ):
    ...
    def _compute_l10n_ec_identification_type(self):
        ...
        for move in self:
            ...
            if (
                ...
                or not move.partner_id.commercial_partner_id.l10n_ec_type_sri
            ):
                continue
            ...
            if move.move_type in ("in_invoice", "in_refund"):
                if move.partner_id.commercial_partner_id.l10n_ec_type_sri == "Ruc":
                    ...
                    move.partner_id.commercial_partner_id.l10n_ec_type_sri
                    == "Pasaporte"
                ):
    ...
    def _check_electronic_authorization_supplier(self):
        ...
        for rec in self.filtered(lambda x: x.company_id.country_id.code == "EC"):
            if (
                ...
                and rec.l10n_ec_type_emission == "electronic"
            ):
                if len(rec.l10n_ec_electronic_authorization) not in (37, 49):
                    raise ValidationError(
                        _(
                            "The electronic authorization number is incorrect, "
                            "this must be 37 or 49 digits. Check the invoice %(l10n_latam_document_number)s of supplier %(partner)s"
                        )
                        % {
                ...
                if not re.match(
                    string_electronic_authorization,
                    rec.l10n_ec_electronic_authorization,
                ):
                    raise ValidationError(
                        _(
                            "The electronic authorization must have only numbers, "
                            "please check the invoice %(l10n_latam_document_number)) of supplier %(partner)s!"
                        )
                        % {
                            "l10n_latam_document_number": rec.l10n_latam_document_number,
                            "partner": rec.partner_id.display_name,
    ...
    def _onchange_partner_id(self):
        ...
        if self.partner_id:
            ...
            if self.partner_id.l10n_ec_foreign and self.move_type in [
                "in_invoice",
                "in_refund",
            ]:
    ...
    def _onchange_l10n_ec_document_number_out(self):
        ...
        if self.l10n_ec_point_of_emission_id:
            l10n_latam_document_number = (
                self.l10n_ec_point_of_emission_id.complete_number(
                    self.l10n_latam_document_number
                )
            )
            # no provocar el inverse nuevamente si el valor del campo es el mismo
    ...
    def _inverse_l10n_latam_document_number(self):
        ...
        super(
            AccountMove,
            self.with_context(
                l10n_ec_foreign=len(self) == 1 and self.l10n_ec_foreign or False
            ),
    ...
    def _onchange_point_of_emission(self):
        for move in self.filtered(
            lambda x: x.company_id.country_id.code == "EC"
            and x.move_type in ("out_invoice", "out_refund", "in_invoice")
        ):
                ...
                if invoice_type in (
                    ...
                    if invoice_type not in ["in_invoice"]:
                        ...
                        (
                            next_number,
                            auth_line,
                        ) = move.l10n_ec_point_of_emission_id.with_context(
                            doc_ids=self.ids
                        ).get_next_value_sequence(
                        ...
                        if next_number:
                            try:
                                ...
                                new_agency, new_printer_point, _n = next_number.split(
                                    "-"
                                ) or (False, False, False)
                                if (
                                    current_agency != new_agency
                                    or current_printer_point != new_printer_point
                                ):
    ...
    def fields_view_get(
        ...
        if (
            ...
            and "invoice_line_ids" in res["fields"]
        ):
            ...
            for ind in [1, 2, 3]:
                ...
                field_string = company[
                    "l10n_ec_string_ride_detail%s" % ind
                ] or "Informacion Adicional %s" % (ind)
                modifiers = {
    ...
    def copy_data(self, default=None):
        ...
        if self.filtered(
            lambda x: x.company_id.country_id.code == "EC"
        ) and not default.get("l10n_latam_document_number"):
    ...
    def action_cancel_invoice_sent_email(self):
        ...
        try:
            ...
            _logger.debug("error sending email", e)
        return
    ...
    def _l10n_ec_get_extra_domain_move(self):
        ...
        if self.env.context.get(
            "filter_original_invoice_type"
        ) and self.env.context.get("original_invoice_type"):
            invoice_type = (
                "out_invoice"
                if self.env.context.get("original_invoice_type") == "out_refund"
                else "in_invoice"
            )
    ...
    def _search(
        ...
        res = super(AccountMove, self)._search(
    ...
    def _read_group_raw(
        ...
        res = super(AccountMove, self)._read_group_raw(
    ...
    def button_draft(self):
        ...
        for move in self:
            ...
            withhold_electronic = move.l10n_ec_withhold_ids.filtered(
                ...
                and x.l10n_ec_xml_data_id.state not in ["draft", "signed", "cancel"]
            )
            if (
                ...
                action = self.env.ref(
                    "l10n_ec_niif.action_wizard_cancel_electronic_documents_form_view"
                ).read()[0]
                action["context"] = {
            ...
            if (
                ...
                and move.l10n_ec_xml_data_id.state
                not in ["draft", "signed", "cancel", "returned"]
                and not self.env.context.get("cancel_electronic_document", False)
            ):
                if move.company_id.l10n_ec_request_sri_validation_cancel_doc:
                    client_ws = xml_data.get_current_wsClient("2", "authorization")
                    if move._l10n_ec_is_document_authorized_in_sri(
                        client_ws, move.l10n_ec_xml_data_id.l10n_ec_xml_key
                    ):
                        raise UserError(
                            _(
        ...
        for move in self:
            if move.company_id.country_id.code == "EC":
                ...
                if (
                    ...
                    raise UserError(
                        _(
                            "The document %(l10n_latam_document_number)s has the code %(l10n_latam_document_type_id)s in the document type, but a "
                            "refund has been included, so it must be code 41."
                        )
                        % {
                            "l10n_latam_document_number": move.l10n_latam_document_number,
                            "l10n_latam_document_type_id": move.l10n_latam_document_type_id.code,
                        }
                    )
    ...
    def _check_document_values_for_ecuador(self):
        ...
        if self.l10n_ec_consumidor_final:
            if (
                ...
                raise UserError(
                    _(
                        "You can't make invoice where amount total %(amount_total)s "
                        "is bigger than %(l10n_ec_consumidor_final_limit)s for final customer"
                    )
                    % {
                        "amount_total": self.amount_total,
                        "l10n_ec_consumidor_final_limit": self.company_id.l10n_ec_consumidor_final_limit,
                    }
                )
        ...
        if self.l10n_ec_invoice_type in ("in_invoice", "in_refund", "debit_note_in"):
            ...
            if self.l10n_ec_type_emission in ("pre_printed", "auto_printer"):
                if self.l10n_ec_supplier_authorization_id:
                    ...
                    not self.l10n_ec_supplier_authorization_number
                    and not self.l10n_ec_foreign
                ):
                    raise UserError(
                        _(
        ...
        if (
            ...
            if (
                float_compare(
                    self.amount_total,
                    self.l10n_ec_original_invoice_id.amount_total,
                    precision_digits=2,
                )
            ) == 1:
                raise UserError(
                    _(
                        "The total amount: %(amount_total)s of the credit note: %(document_number)s, "
                        "cannot be greater than the total amount: %(origin_amount_total)s of the invoice %(origin_document_number)s"
                    )
                    % {
                        "amount_total": formatLang(
                            self.env,
                            currency.round(self.amount_total),
                            currency_obj=currency,
                        ),
                        "document_number": self.l10n_ec_get_document_number(),
                        "origin_amount_total": formatLang(
                            self.env,
                            currency.round(
                                self.l10n_ec_original_invoice_id.amount_total
                            ),
                        ...
                        "origin_document_number": self.l10n_ec_original_invoice_id.l10n_ec_get_document_number(),
                    }
                )
            ...
            if self.l10n_ec_original_invoice_id.invoice_payment_state != "paid":
                if (
                    float_compare(
                        self.amount_total,
                        self.l10n_ec_original_invoice_id.amount_residual,
                        precision_digits=2,
                    )
                    == 1
                ):
                    raise UserError(
                        _(
                            "The total amount: %(amount_total)s of the credit note %(document_number)s, "
                            "cannot be greater than the amount residual: %(origin_amount_total)s of the invoice %(origin_document_number)s."
                        )
                        % {
                            "amount_total": formatLang(
                                self.env,
                ...
                credit_notes_recs = self.search(
                    [
                        (
                            "l10n_ec_original_invoice_id",
                            "=",
                            self.l10n_ec_original_invoice_id.id,
                        ),
                        ("id", "!=", self.id),
                        ("state", "=", "posted"),
                    ]
                ...
                if credit_notes_recs:
                    ...
                    if (
                        ...
                        raise UserError(
                            _(
                                "The total amount of all credit notes: %(total)s, "
                                "cannot be greater than the total amount: %(origin_amount_total)s of the invoice %(document_number)s"
                            )
                            % {
                                "total": formatLang(
                                    self.env,
                                ...
                                "document_number": self.l10n_ec_original_invoice_id.l10n_ec_get_document_number(),
                            }
                        )
        ...
        if self.move_type == "in_invoice" and not self.env.context.get(
            "skip_sri_validations_in_invoice"
        ):
            if (
                ...
                error_list.append(
                    _(
                        "You must select the fiscal support to validate invoices %(document_number)s of supplier %(partner)s."
                    )
                    % {
                ...
                if self.partner_id.country_id.code == "EC":
                    if self.l10n_latam_document_type_id.code == "41":
                        if rent_withhold_taxes or withhold_iva_taxes:
                            error_list.append(
                                _(
                                    "You cant not apply withholding for document types: %s"
                                )
                                % self.l10n_latam_document_type_id.name
                            )
                    else:
                        require_withhold = True
                        if (
                            self.company_id.partner_id.property_account_position_id
                            and not self.company_id.partner_id.property_account_position_id.l10n_ec_check_withhold
                        ):
        ...
        current_xml_recs = self.filtered(
            lambda x: x.l10n_ec_xml_data_id
            and (
                x.l10n_ec_xml_data_id.state == "cancel"
                and x.l10n_ec_xml_data_id.authorization_to_cancel
            )
        ).mapped("l10n_ec_xml_data_id")
        if current_xml_recs:
            error_list.append(
                _("You cannot validate this record, already is cancelled on the SRI")
            )
        if error_list:
            raise UserError("\n".join(error_list))
        return True
        ...
        if self.l10n_ec_invoice_type in ("in_invoice", "in_refund", "debit_note_in"):
            ...
            if self.l10n_ec_type_emission == "auto_printer":
                ...
                if (
                    ...
                    and self.l10n_latam_document_type_id.code != "01"
                ):
                    self.write({"l10n_ec_sri_authorization_state": "valid"})
                    return True
                if self.l10n_ec_supplier_authorization_id:
                ...
                try:
                    limit_days = int(
                        ...
                        .sudo()
                        .get_param("sri.days.to_validate_documents", 5)
                    )
                except Exception as ex:
                    limit_days = 5
                    _logger.error(
    ...
    def validate_quantity_move_line(self):
        ...
        for move in self:
            if move.l10n_ec_invoice_type in (
                "in_invoice",
                "out_invoice",
                "in_refund",
                "out_refund",
    ...
    def _prepare_withhold_values(self):
        ...
        if (
            ...
            (
                next_number,
                auth_line,
            ) = self.l10n_ec_point_of_emission_withhold_id.get_next_value_sequence(
        ...
        for tax in tax_data.keys():
            ...
            if not line_with_taxes:
                ...
                tax_values = tax.compute_all(
                    sum(lines_with_tax.mapped("price_subtotal"))
                )
                for tax_vals in tax_values.get("taxes", []):
                    ...
                    if tax_vals.get("tax_repartition_line_id"):
                        tax_repartition_lines = self.env[
                            "account.tax.repartition.line"
                        ].browse(tax_vals.get("tax_repartition_line_id"))
    ...
    def l10n_ec_get_invoice_type(self):
        ...
        return modules_mapping.l10n_ec_get_invoice_type(
            self.move_type, internal_type, False
        )
    ...
    def l10n_ec_validate_fields_required_fe(self):
        ...
        if not self.commercial_partner_id.country_id:
            message_list.append(
                _("Must be configure Country to Partner: %s.")
                % (self.commercial_partner_id.name)
            )
    ...
    def l10n_ec_action_create_xml_data(self):
        ...
        current_xml_recs = self.filtered(
            lambda x: x.l10n_ec_xml_data_id
            and x.l10n_ec_xml_data_id.state not in ("draft", "authorized")
        ).mapped("l10n_ec_xml_data_id")
        if current_xml_recs:
    ...
    def _l10n_ec_get_invoice_lines_to_fe(self):
        ...
        for line in invoice_lines:
            if line.tax_ids not in discount_applied_data:
                discount_applied_data[line.tax_ids] = {
                    "lines": self.env["account.move.line"],
                    "discount_applied": 0,
                }
            discount_applied_data[line.tax_ids]["lines"] |= line
            discount_lines = (
                discount_by_tax.get(line.tax_ids) or self.env["account.move.line"]
            )
            total_discount_amount = abs(sum(discount_lines.mapped("price_subtotal")))
            ail_by_tax = (
                invoice_lines_by_tax.get(line.tax_ids) or self.env["account.move.line"]
            )
            ail_amount_total = abs(sum(ail_by_tax.mapped("price_subtotal")))
            discount_unit_additional = 0.0
            if ail_amount_total:
                discount_unit_additional = round(
                    (line.price_subtotal / ail_amount_total) * 100.0, 2
                )
            discount = round(
                ((line.price_unit * line.quantity) * ((line.discount or 0.0) / 100)), 2
            )
            discount_additional = round(
                (total_discount_amount * ((discount_unit_additional or 0.0) / 100)), 2
            )
            ...
            if len(discount_applied_data[line.tax_ids]["lines"]) == len(ail_by_tax):
                discount_additional = round(
                    ...
                    - discount_applied_data[line.tax_ids]["discount_applied"],
                    2,
                )
            discount_applied_data[line.tax_ids]["discount_applied"] += round(
                discount_additional, 2
            )
    ...
    def l10n_ec_asign_discount_to_lines(self):
        for invoice in self:
            ...
            for line in invoice_lines:
                ...
                line.write(
                    {
                        "l10n_ec_discount_additional": line_data.get(
                            "discount_additional"
                        )
                        or 0.0
                    }
        ...
        if xml_version.version_file in ("2.0.0", "2.1.0"):
            ...
            if other_values and len(other_values.keys()) == 1 and len(other_taxes) == 1:
                ...
                for name in other_values.keys():
                    ...
                    SubElement(rubro, "total").text = util_model.formato_numero(
                        other_values[name], 2
                    )
    ...
    def l10n_ec_get_info_liquidation(self, node):
        ...
        SubElement(
            ...
            self.amount_total
            + sum(self.l10n_ec_withhold_line_ids.mapped("tax_amount_currency")),
            decimales=currency.decimal_places,
        )
        SubElement(
    ...
    def _check_invoice_date(self):
        for move in self:
            ...
            if (
                ...
                raise UserError(
                    _(
                        "You cannot create an electronic document with a date later than the current one"
                    )
                )
            if (
                ...
                raise UserError(
                    _(
                        "You cannot create the purchase withholding electronic %s with a date later than the current one"
                    )
            ...
            if (
                ...
                raise UserError(
                    _(
                        "You cannot create the purchase liquidation electronic %s with a date later than the current one"
                    )
                    % move.l10n_latam_document_number
                )
    ...
    def l10n_ec_generate_info_reembolsos_head_xml(self, parent_node):
        ...
        SubElement(
            parent_node, "totalComprobantesReembolso"
        ).text = util_model.formato_numero(
            sum(r.total_invoice for r in self.l10n_ec_refund_ids),
            decimales=currency.decimal_places,
        )
        SubElement(
            parent_node, "totalBaseImponibleReembolso"
        ).text = util_model.formato_numero(
        ...
        SubElement(
            parent_node, "totalImpuestoReembolso"
        ).text = util_model.formato_numero(
            sum(r.total_iva for r in self.l10n_ec_refund_ids)
...
class AccountMoveLine(models.Model):
    ...
    def _get_third_amounts_line(self):
        ...
        if len(self.tax_ids.filtered(lambda x: x.tax_group_id.id == other_tax.id)) == 1:
            other_tax = self.tax_ids.filtered(
                lambda x: x.tax_group_id.id == other_tax.id
            )
