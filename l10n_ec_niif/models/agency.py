...
class L10EcPointOfEmission(models.Model):
    ...
    def _compute_complete_name(self):
        for printer_point in self:
            complete_name = f"{printer_point.agency_id.number or ''}-{printer_point.number or ''} {printer_point.name or ''}"
    ...
    def get_authorization_for_number(
        ...
        if self.type_emission in ("pre_printed", "auto_printer") or force_preprint:
            if not doc_find:
                raise UserError(
                    _(
                        "It is not possible to find authorization for the document type %(model_description)s "
                        "at the point of emission %(number)s for the agency %(agency_id)s with date %(emission_date)s on company: %(company)s"
                    )
                    % {
                        "model_description": model_description,
                        "number": self.number,
                        "agency_id": self.agency_id.number,
        ...
        if self.type_emission in ("pre_printed", "auto_printer") or force_preprint:
            if not doc_finded and raise_exception:
                raise UserError(
                    _(
                        "It is not possible to find authorization for the document type %(model_description)s "
                        "at the point of issue %(number)s for the agency %(agency_id)s with date %(date)s"
                    )
                    % {
                        "model_description": model_description,
                        "number": self.number,
            ...
            if not next_seq and first_number_electronic and self:
                ...
                if self.env.context.get("doc_ids", []):
                    domain += [("id", "not in", self.env.context.get("doc_ids", []))]
                start_doc_number = "{}-{}-{}".format(
                    self.agency_id.number, self.number, "%"
                )
...
class L10EcPointOfEmissionDocumentSequence(models.Model):
    ...
    initial_sequence = fields.Integer(required=True, default=1)
