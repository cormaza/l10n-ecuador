...
class L10nEcWithhold(models.Model):
    ...
    issue_date = fields.Date(
        readonly=True, states=_STATES, required=True, tracking=True
    )
    ...
    concept = fields.Char(readonly=True, states=_STATES, required=False, tracking=True)
    note = fields.Char(required=False)
    ...
    move_count = fields.Integer(compute="_compute_l10n_ec_withhold_ids", store=False)
    ...
    def _compute_access_url(self):
        res = super(L10nEcWithhold, self)._compute_access_url()
        for withhold in self:
    ...
    def _check_no_retention_same_invoice(self):
        for rec in self:
            ...
            if withhold_find:
                raise UserError(
                    _(
                        "Withholding: %(number)s, with partner: %(partner)s. Invoice: %(invoice)s is already registered on other retention: %(withhold)s"
                    )
                    % {
                        "number": rec.number,
                        "partner": rec.partner_id.name,
                        "invoice": rec.invoice_id.display_name,
                        "withhold": withhold_find.number,
                    }
                )
        return True
            ...
            if rec.type in ["sale", "credit_card"]:
                ...
                if self.type == "sale":
                    ...
                    if not rec.company_id.l10n_ec_withhold_iva_credit_card_account_id:
                        raise UserError(
                            _(
                                "You must configure Withhold Vat Account for Credit Card on Company to continue"
                            )
                        )
                    if not rec.company_id.l10n_ec_withhold_rent_credit_card_account_id:
                        raise UserError(
                            _(
                                "You must configure Withhold Rent Account for Credit Card on Company to continue"
                            )
                        )
    ...
    def _create_account_move(self):
        ...
        model_aml = self.env["account.move.line"].with_context(
        ...
        for line in self.line_ids:
            ...
            if line.tax_amount_currency > 0:
                debit_vals, credit_vals = self._prepare_move_line(
                    move_rec, line, destination_account
                )
    ...
    def _prepare_move_vals(self):
        ...
        return {
            ...
            "currency_id": self.line_ids.mapped("currency_id").id
    ...
    def _prepare_move_line(self, move, line, destination_account):
        ...
        invoice_number = (
            ...
            or self.l10n_ec_legacy_document_number
            or ""
        )
        if line.type == "iva":
            ...
            name = _("Withhold Vat. %(number)s - %(invoice_number)s") % {
            ...
            tax_code = []
            name = _("Withhold Rent Tax. %(number)s - %(invoice_number)s") % {
                "number": self.display_name,
                "invoice_number": invoice_number,
            }
        debit_vals = {
