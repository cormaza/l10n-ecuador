...
class L10nECSriAuthorizationSupplier(models.Model):
    ...
    def _check_dates(self):
        for auth in self:
            if auth.start_date >= auth.expiration_date:
                raise ValidationError(
                    _(
                        "The dates of authorization of supplier %(number)s "
                        "must be valid start: %(start_date)s end: %(expiration_date)s"
                    )
                    % {
                        "number": auth.number,
            ...
            if auth.first_sequence >= auth.last_sequence and not auth.autoprinter:
                raise ValidationError(
                    _(
                        ...
                        "please check the authorization %(auth)s start: %(first_sequence)s last: %(last_sequence)s"
                    )
                    % {
                        "auth": auth.display_name,
                        "first_sequence": auth.first_sequence,
                        "last_sequence": auth.last_sequence,
                    }
                )
        ...
        for auth in self:
            ...
            if has_change:
                ...
                if invoice_recs or retention_recs:
                    raise UserError(
                        _(
                            "You cannot modify authorization %(auth)s of partner %(partner)s, "
                            "this is already used in active documents, contact de system administrator"
                        )
                        % {
                            "auth": auth.display_name,
                            "partner": auth.partner_id.display_name,
                        }
                    )
        return True
    ...
    def check_number_document(
        ...
        if not number:
            raise UserError(
                _("You must declare document number to check authorization")
            )
        if (
            not authorization
            and not foreign
        ...
        if not foreign:
            ...
            if date > authorization.expiration_date or date < authorization.start_date:
                raise UserError(
                    _(
                        "%(date)s is not within the authorization dates %(start_date)s y %(expiration_date)s"
                    )
                    % {
                        "date": date,
            ...
            if num_shop != authorization.agency:
                raise UserError(
                    _(
                        "The agency number %(num_shop)s does not correspond to the authorization, that should be %(agency)s"
                    )
                    % {"num_shop": num_shop, "agency": authorization.agency}
                )
            if num_printer != authorization.printer_point:
                raise UserError(
                    _(
                        "The emission point number %(num_printer)s does not correspond to the authorization, that should be %(printer_point)s"
                    )
                    % {
                        "num_printer": num_printer,
                        "printer_point": authorization.printer_point,
                    }
                )
            if (
                ...
                raise UserError(
                    _(
                        "The sequence number %(num_doc)s is not within the range %(first_sequence)s and %(last_sequence)s"
                    )
                    % {
                        "num_doc": num_doc,
                        "first_sequence": authorization.first_sequence,
                        "last_sequence": authorization.last_sequence,
                    }
                )
        if number:
            if partner_id:
        ...
        if not foreign:
            ...
            if list_ids_exist and partner:
                raise UserError(
                    _(
                        "There is another document of type %(description_name)s with number '%(number)s' for partner %(partner)s"
                    )
                    % {
    ...
    def validate_unique_document_partner(
        ...
        if model_recs:
            raise UserError(
                _(
                    "There's another document type %(model_description)s with number '%(number)s' for partner %(partner)s"
                )
                % {
    ...
    def get_supplier_authorizations(
        ...
        if not auth_recs:
            ...
            if not auth_recs:
                if not seq_number:
                    message += _(
                        "There's no exist authorization for document type %(model_description)s for partner %(partner)s with date %(date)s, "
                    ...
                    message += _(
                        "There's no exist authorization for document type %(model_description)s "
                        "for partner %(partner)s with date %(date)s for sequence %(seq_number)s, "
                        "check if you should create authorization"
                    ) % {
                        "model_description": model_description,
                        "partner": partner.name,
        ...
        if len(auth_recs) > 1:
            message += _(
                "There is more than one match of active authorizations for the partner %(partner)s of type %(model_description)s "
                "for the number %(number)s with date %(date)s, please select the correct "
    ...
    def validate_authorization_into_sri(
        ...
        try:
            ...
            _logger.info("Error Validating authorization into sri:", e)
