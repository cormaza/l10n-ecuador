...
class ResPartner(models.Model):
            ...
            if partner.country_id:
                ...
                if (
                    ...
                    partner.property_account_payable_id = (
                        partner.env.company.partner_id.property_account_payable_id.id
                    )
            if l10n_ec_foreign:
    ...
    def set_accounting_account_foreign(self):
        ...
        accounting_account_receivable_fireign_id = (
            ...
            .sudo()
            .get_param("l10n_ec_accounting_account_receivable_fireign_id")
        )
        accounting_account_payable_fireign_id = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("l10n_ec_accounting_account_payable_fireign_id")
        )
        if accounting_account_receivable_fireign_id:
            self.property_account_receivable_id = int(
                accounting_account_receivable_fireign_id
            )
        if accounting_account_payable_fireign_id:
            self.property_account_payable_id = int(
                accounting_account_payable_fireign_id
            )
    ...
    l10n_ec_show_force_validate = fields.Boolean(
        string="Show force validate?", copy=False, default=False
    )
    # campo para permitir saltar la validacion de RUC solo bajo la condicion que no pase el modulo 11 y el SRI este caido
    l10n_ec_force_validate_nif = fields.Boolean(
        string="Force validate nif?", copy=False
    )
    ...
    def _get_partner_info_from_sri(self, vat):
        ...
        try:
            response = requests.get(
                f"https://srienlinea.sri.gob.ec/movil-servicios/api/v1.0/estadoTributario/{vat}"
            )
            data = response.json()
        except Exception as e:
            data = {}
            ...
            if partner.country_id:
                if partner.vat and partner.country_id.code == "EC":
                    try:
                        ...
                        _logger.debug(
                            _("Error checking vat: %(vat)s error:%(error)s")
                            % {"partner": partner.vat, "error": str(e)}
                        )
    ...
    def create(self, vals):
        ...
        if (
            ...
            for val in vals:
                if not val.get("l10n_latam_identification_type_id", False) and val.get(
                    "vat"
                ):
    ...
    def _check_l10n_ec_values(self):
        ...
        if self.l10n_latam_identification_type_id.id not in (
            ...
            raise UserError(
                _(
                    "You must set Identification type as RUC, Cedula or Passport for ecuadorian company, on partner %s"
                )
                % (self.name,)
            )
