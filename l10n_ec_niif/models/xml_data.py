...
class SriXmlData(models.Model):
    ...
    l10n_ec_xml_key = fields.Char(
        "Clave de Acceso", size=49, readonly=True, index=True, tracking=True
    )
    ...
    def generate_info_tributaria(self, node, document, environment, company):
        ...
        if company.l10n_ec_microenterprise_regime_taxpayer:
            SubElement(
                infoTributaria, "regimenMicroempresas"
            ).text = "CONTRIBUYENTE RÉGIMEN MICROEMPRESAS"
        ...
        if company.l10n_ec_rimpe_regime and False:
            ...
            ).text = "CONTRIBUYENTE RÉGIMEN RIMPE"
    ...
    def action_sing_xml_file(self):
        for xml_rec in self:
            ...
            try:
                ...
                raise UserError(tools.ustr(ex)) from ex
        return True
    ...
    def process_document_electronic(self, force_send_now=False):
        ...
        if xml_process_online:
            ...
            if xml_to_sign:
                ...
                xml_to_sign.filtered(
                    lambda x: x.state == "signed"
                ).action_send_xml_file()
    ...
    def send_documents_rejected(self):
        ...
        template_mail_docs_no_autorization = self.env.ref(
            "l10n_ec_niif.mail_documents_electronic_rejected"
        )
        for company in all_companies:
            ...
            if xml_rejected:
                template_mail_docs_no_autorization.with_context(
    ...
    def action_cancel(self):
        for xml_data in self:
            if xml_data.state == "authorized" and not xml_data.authorization_to_cancel:
                raise UserError(
                    _("You can't cancel document: %s is authorized on SRI")
                    % (xml_data.display_name,)
                )
        self.write(
            {
