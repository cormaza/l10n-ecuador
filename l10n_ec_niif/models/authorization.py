...
class L10nECSriAuthorizationLine(models.Model):
    ...
    document_type = fields.Selection(selection="_get_available_type", required=True)
    ...
    padding = fields.Integer(default=9)
    count_invoice = fields.Integer(related="authorization_id.count_invoice")
        ...
        for line in self:
            ...
            if line.last_sequence <= line.first_sequence:
                raise ValidationError(
                    _(
                        "The first sequence %(first_sequence)s must be lower than last sequence %(last_sequence)s"
                    )
                    % {
                        "first_sequence": line.first_sequence,
                        "last_sequence": line.last_sequence,
                    }
                )
    ...
    def _check_document_type(self):
        for line in self:
            ...
            if other_recs:
                raise ValidationError(
                    _(
                        "There's another line with document type %(document_name)s "
                        "for point of emission %(point_of_emission_id)s on agency %(agency_id)s to authorization %(authorization_id)s"
                    )
                    % {
                        "document_name": _DOCUMENT_NAMES.get(line.document_type),
                        "point_of_emission_id": self.point_of_emission_id.display_name,
                        "agency_id": self.agency_id.display_name,
                        "authorization_id": self.authorization_id.display_name,
                    }
                )
            ...
            if other_recs:
                ...
                if not valid:
                    raise ValidationError(
                        _(
                            "There's another line with document type %(document_name)s for "
                            "point of emission %(point_of_emission_id)s in the agency %(agency_id)s, please check sequences"
                        )
                        % {
                            "document_name": _DOCUMENT_NAMES.get(line.document_type),
                            "point_of_emission_id": self.point_of_emission_id.display_name,
    ...
    def validate_unique_value_document(
        ...
        if model_recs:
            ...
            if self.env.context.get("from_constrain", False):
                raise ValidationError(
                    _(
                        "There is another document type %(model_description)s with number '%(document_number)s' for the company %(company)s"
                    )
                    % {
