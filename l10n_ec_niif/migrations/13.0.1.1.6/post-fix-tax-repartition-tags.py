    ...
    for company in all_company:
        for tax_idxml, repartition_data in tax_list:
            try:
                ...
                current_tax_moves = MoveLines.search(
                    [("tax_line_id", "=", current_tax.id)]
                )
                current_base_moves = MoveLines.search(
                    [("tax_ids", "in", current_tax.ids)]
                )
                invoice_base_tags_ids = (
                    current_tax.invoice_repartition_line_ids.filtered(
                ...
                if current_tax_moves:
                    refund_moves = current_tax_moves.filtered(
                        lambda x: x.move_id.type in ("out_refund", "in_refund")
                    )
                    normal_moves = current_tax_moves - refund_moves
                    if normal_moves:
                        normal_moves.write(
                            {
                                "tag_ids": invoice_tax_tags_ids
                                and [(6, 0, invoice_tax_tags_ids)],
                            }
                        )
                    if refund_moves:
                        refund_moves.write(
                            {
                                "tag_ids": refund_tax_tags_ids
                                and [(6, 0, refund_tax_tags_ids)],
                            }
                        )
                if current_base_moves:
                    refund_moves = current_base_moves.filtered(
                        lambda x: x.move_id.type in ("out_refund", "in_refund")
                    )
                    ...
                    if normal_moves:
                        normal_moves.write(
                            {
                                "tag_ids": invoice_base_tags_ids
                                and [(6, 0, invoice_base_tags_ids)],
                            }
                        )
                    if refund_moves:
                        refund_moves.write(
                            {
                                "tag_ids": refund_base_tags_ids
                                and [(6, 0, refund_base_tags_ids)],
                            }
                        )
